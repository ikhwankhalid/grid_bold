...
dx = 1
tau_rep = settings.tau_rep / 2
w_rep = settings.w_rep / 2
...
def mfunc(i):
    ...
    h_rep_star = (get_hexsym2(summed_fr, trajec_star), get_pathsym(trajec_star), np.mean(meanfr[~np.isnan(meanfr).any()]))
    ...
    h_conj_star = (get_hexsym2(summed_fr, trajec_star), get_pathsym(trajec_star), np.mean(meanfr[~np.isnan(meanfr).any()]))
    ...
    h_cl_star = (get_hexsym2(summed_fr, trajec_star), get_pathsym(trajec_star), np.mean(meanfr[~np.isnan(meanfr).any()]))
...
# os.makedirs(os.path.dirname(os.path.join(settings.loc, "summary", 'summary_figure_real.png')), exist_ok=True)
# plt.savefig(os.path.join(settings.loc, "summary", 'summary_figure_real.png'), dpi=300)
# plt.close(fig)
...
np.save(os.path.join(settings.loc, "summary",'summary_bar_plot_3hyp3traj_real'), data)
