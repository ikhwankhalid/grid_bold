...
def grid3(
    ...
    if np.size(offs) == 3:
        ret = rec(
            np.cos(2*np.pi*sc*np.sin(angle*np.pi/180)*(X-offs[0])+2*np.pi*sc*np.cos(angle*np.pi/180)*(Y-offs[1]))
        ) * \
        rec(
            np.cos(2*np.pi*sc*np.sin((angle+60)*np.pi/180)*(X-offs[0])+2*np.pi*sc*np.cos((angle+60)*np.pi/180)*(Y-offs[1]))
        ) * \
        rec(
            np.cos(2*np.pi*sc*np.sin((angle+120)*np.pi/180)*(X-offs[0])+2*np.pi*sc*np.cos((angle+120)*np.pi/180)*(Y-offs[1]))
        )
        return ret
    else:
        assert len(offs[0]) == len(offs[1]), "ox and oy must have same length"
        ret = rec(
            np.cos(2*np.pi*sc*np.sin(angle*np.pi/180)*(X[:,:,None]-(offs[0])[None,None,:])+2*np.pi*sc*np.cos(angle*np.pi/180)*(Y[:,:,None]-(offs[1])[None,None,:]))
        ) * \
        rec(
            np.cos(2*np.pi*sc*np.sin((angle+60)*np.pi/180)*(X[:,:,None]-(offs[0])[None,None,:])+2*np.pi*sc*np.cos((angle+60)*np.pi/180)*(Y[:,:,None]-(offs[1])[None,None,:]))
        ) * \
        rec(
            np.cos(2*np.pi*sc*np.sin((angle+120)*np.pi/180)*(X[:,:,None]-(offs[0])[None,None,:])+2*np.pi*sc*np.cos((angle+120)*np.pi/180)*(Y[:,:,None]-(offs[1])[None,None,:]))
        )
        return ret
...
def create_randomfield_3D(n,mode='grid'):
    ...
    if mode == "none":
        ...
        if mode=='gauss':
            ...
            # kernel = np.reshape(grid2(X[:,0],X[:,1],sc=1/grsc * 2/np.sqrt(3)),(int(n/part),int(n/part),int(n/part)))
            # kernel[np.sqrt(gridx**2 + gridy**2 + gridz**2)>1.5*grsc] = 0
            # kernel = kernel/np.sum(kernel)/(3/(n-1))**2
        else:
            raise Exception('What are you doing?')
        #plt.figure()
...
def reproduce_gu2(ph1,ph2,x,y,z, reps=100000000):
    ...
    phdist1 = phfr1[ind1].reshape(-1, 1) - (phfr1[ind2].reshape(-1, 1)  + xsfr.reshape(1, -1) )
    phdist2 = phfr2[ind1].reshape(-1, 1) - (phfr2[ind2].reshape(-1, 1)  + ysfr.reshape(1, -1) )
    # phdistr1, phdistr2 = convert_to_rhombus(phdist1,phdist2)
...
#     output = np.array(
#         [np.mod(vec1-vec2, 2*np.pi), np.mod(vec2-vec1, 2*np.pi)]
#     ).min(axis=0)
#     return output
...
def mfunc(i):
    ph1n,ph2n,x,y,z,kerneln = create_randomfield_3D(n=ngridcells,mode="none")
    ph1,ph2,x,y,z,kernel = create_randomfield_3D(n=ngridcells,mode='grid')
    ph1c,ph2c,xc,yc,zc,kernelc = create_randomfield_3D(n=ngridcells,mode='gauss')
    kx_none,ky_none = vonmises.fit(2*np.pi*np.ravel(ph1n), fscale=1)[0], vonmises.fit(2*np.pi*np.ravel(ph2n), fscale=1)[0]
    kx_grid,ky_grid = vonmises.fit(2*np.pi*np.ravel(ph1), fscale=1)[0], vonmises.fit(2*np.pi*np.ravel(ph2), fscale=1)[0]
    kx_gauss,ky_gauss = vonmises.fit(2*np.pi*np.ravel(ph1c), fscale=1)[0], vonmises.fit(2*np.pi*np.ravel(ph2c), fscale=1)[0]

    kx_none,ky_none = vonmises.fit(2*np.pi*np.random.choice(np.ravel(ph1n), size=int(realncells**3), replace=False), fscale=1)[0], vonmises.fit(2*np.pi*np.random.choice(np.ravel(ph2n), size=int(realncells**3), replace=False), fscale=1)[0]
    kx_grid,ky_grid = vonmises.fit(2*np.pi*np.random.choice(np.ravel(ph1), size=int(realncells**3), replace=False), fscale=1)[0], vonmises.fit(2*np.pi*np.random.choice(np.ravel(ph2), size=int(realncells**3), replace=False), fscale=1)[0]
    kx_gauss,ky_gauss = vonmises.fit(2*np.pi*np.random.choice(np.ravel(ph1c), size=int(realncells**3), replace=False), fscale=1)[0], vonmises.fit(2*np.pi*np.random.choice(np.ravel(ph2c), size=int(realncells**3), replace=False), fscale=1)[0]
...
def mfunc_nkappa(i):   
    ...
    for ik, ncells in enumerate(ncellslist):
        kx_none[ik],ky_none[ik] = vonmises.fit(2*np.pi*np.random.choice(np.ravel(ph1n), size=int(ncells**3), replace=False), fscale=1)[0], vonmises.fit(2*np.pi*np.random.choice(np.ravel(ph2n), size=int(ncells**3), replace=False), fscale=1)[0]
        kx_grid[ik],ky_grid[ik] = vonmises.fit(2*np.pi*np.random.choice(np.ravel(ph1), size=int(ncells**3), replace=False), fscale=1)[0], vonmises.fit(2*np.pi*np.random.choice(np.ravel(ph2), size=int(ncells**3), replace=False), fscale=1)[0]
        kx_gauss[ik],ky_gauss[ik] = vonmises.fit(2*np.pi*np.random.choice(np.ravel(ph1c), size=int(ncells**3), replace=False), fscale=1)[0], vonmises.fit(2*np.pi*np.random.choice(np.ravel(ph2c), size=int(ncells**3), replace=False), fscale=1)[0]
...
if not os.path.isfile(ncells_kappas_fname):
    ...
    ncellsdata = Parallel(
        n_jobs=-1, verbose=100)(delayed(mfunc_nkappa)(i) for i in tqdm(range(rep))
    )
...
print(ncellsdata.shape)

kx_none_ncells,ky_none_ncells,kx_gauss_ncells,ky_gauss_ncells,kx_grid_ncells,ky_grid_ncells = ncellsdata

# ii=0
# kx_grid = np.zeros(rep)
# ky_grid = np.zeros(rep)
# kz_grid = np.zeros(rep)
# kx_gauss = np.zeros(rep)
# ky_gauss = np.zeros(rep)
# kz_gauss = np.zeros(rep)
# while ii<rep:
#     print(ii)
#     ph1,ph2,ph3,x,y,z,kernel = create_randomfield_3D(n=200,mode='grid')
#     ph1c,ph2c,ph3c,xc,yc,zc,kernelc = create_randomfield_3D(n=200,mode='gauss')
#     kx_grid[ii],ky_grid[ii],kz_grid[ii] = vonmises.fit(2*np.pi*np.ravel(ph1), fscale=1)[0], vonmises.fit(2*np.pi*np.ravel(ph2), fscale=1)[0], vonmises.fit(2*np.pi*np.ravel(ph3), fscale=1)[0]
#     kx_gauss[ii],ky_gauss[ii],kz_gauss[ii] = vonmises.fit(2*np.pi*np.ravel(ph1c), fscale=1)[0], vonmises.fit(2*np.pi*np.ravel(ph2c), fscale=1)[0], vonmises.fit(2*np.pi*np.ravel(ph3c), fscale=1)[0]
#     ii+=1
...
plt.imshow(ph1n[:, :, 10], cmap='twilight_shifted', origin="lower")
...
plt.xticks(np.linspace(0, ngridcells, 2, endpoint=True), np.linspace(0, ngridcells, 2, endpoint=True)/ngridcells*3)
plt.ylabel("y (mm)")
plt.xlabel("x (mm)")
...
# plt.yticks(np.linspace(0, ngridcells, 2, endpoint=True), np.linspace(0, ngridcells, 2, endpoint=True)/ngridcells*3)
plt.yticks([ngridcells-1], [3.0])
plt.xticks(np.linspace(0, ngridcells, 2, endpoint=True), np.linspace(0, ngridcells, 2, endpoint=True)/ngridcells*3)
plt.ylabel("y (mm)")
plt.xlabel("x (mm)")
...
plt.imshow(np.sum(kernelc, axis=-1).T, interpolation="gaussian", origin="lower")
...
plt.xticks(np.linspace(0, 500, 6, endpoint=True), np.linspace(0, 500, 6, endpoint=True).astype(int))
...
h_none, _, _ = np.histogram2d(ph1n.flatten(), ph2n.flatten(), bins=(Xc[0, :], Yc[:, 0]))
h_gauss, _, _ = np.histogram2d(ph1c.flatten(), ph2c.flatten(), bins=(Xc[0, :], Yc[:, 0]))
h_grid, _, _ = np.histogram2d(ph1.flatten(), ph2.flatten(), bins=(Xc[0, :], Yc[:, 0]))
...
if not os.path.isfile(phases_eg_fname):
    ...
    kx_none_phases,ky_none_phases = vonmises.fit(2*np.pi*np.ravel(ph1n), fscale=1)[0], vonmises.fit(2*np.pi*np.ravel(ph2n), fscale=1)[0]
    kx_grid_phases,ky_grid_phases = vonmises.fit(2*np.pi*np.ravel(ph1), fscale=1)[0], vonmises.fit(2*np.pi*np.ravel(ph2), fscale=1)[0]
    kx_gauss_phases,ky_gauss_phases = vonmises.fit(2*np.pi*np.ravel(ph1c), fscale=1)[0], vonmises.fit(2*np.pi*np.ravel(ph2c), fscale=1)[0]
    with open(phases_eg_fname, 'wb') as f:
        pickle.dump([kx_none_phases,ky_none_phases,kx_grid_phases,ky_grid_phases,kx_gauss_phases,ky_gauss_phases], f)
else:
    with open(phases_eg_fname, "rb") as f:
        kx_none_phases,ky_none_phases,kx_grid_phases,ky_grid_phases,kx_gauss_phases,ky_gauss_phases = pickle.load(f)
...
im = ax_phasesnone.pcolormesh(Xc,Yc,h_none,shading='auto', vmin=cbarmin, vmax=cbarmax)
plt.plot([0,1],[0,0],'k--')
...
plt.text(0.85, 0.76, f"$\kappa_s$={np.round_(np.mean(np.array([kx_none_phases, ky_none_phases])), 3)}", fontsize=settings.fs, transform=plt.gcf().transFigure, zorder=100)


ax_phasesgauss = fig.add_subplot(spec[1, 5:])
im = ax_phasesgauss.pcolormesh(Xc,Yc,h_gauss,shading='auto', vmin=cbarmin, vmax=cbarmax)
plt.plot([0,1],[0,0],'k--')
plt.plot([0,0.5],[0,np.sqrt(3)/2],'k--')
plt.plot([0.5,1.5],[np.sqrt(3)/2,np.sqrt(3)/2],'k--')
plt.plot([1,1.5],[0,np.sqrt(3)/2],'k--')
...
cb.ax.set_yticklabels(["min", "max"])
cb.set_label('Density', rotation=90)
plt.text(0.85, 0.605, f"$\kappa_s$={np.round_(np.mean(np.array([kx_gauss_phases, ky_gauss_phases])), 3)}", fontsize=settings.fs, transform=plt.gcf().transFigure, zorder=100)


ax_phasesgrid = fig.add_subplot(spec[2, 5:])
im = ax_phasesgrid.pcolormesh(Xc,Yc,h_grid,shading='auto', vmin=cbarmin, vmax=cbarmax)
plt.plot([0,1],[0,0],'k--')
plt.plot([0,0.5],[0,np.sqrt(3)/2],'k--')
plt.plot([0.5,1.5],[np.sqrt(3)/2,np.sqrt(3)/2],'k--')
plt.plot([1,1.5],[0,np.sqrt(3)/2],'k--')
...
cb.ax.set_yticklabels(["min", "max"])
cb.set_label('Density', rotation=90)
plt.text(0.85, 0.445, f"$\kappa_s$={np.round_(np.mean(np.array([kx_grid_phases, ky_grid_phases])), 3)}", fontsize=settings.fs, transform=plt.gcf().transFigure, zorder=100)
...
# ncells kappas
# print(np.hstack((kx_none, ky_none)).shape)
ncellslist = np.linspace(10, 100, npointscells, endpoint=True).astype(int)
...
plt.plot(ncellslist**3, np.mean(k_none_ncells, axis=-1), color="black", label="no kernel")
plt.plot(ncellslist**3, np.mean(k_gauss_ncells, axis=-1), color="black", linestyle="--", label="gaussian kernel")
plt.plot(ncellslist**3, np.mean(k_grid_ncells, axis=-1), color="black", linestyle=":", linewidth=3, label="grid kernel")
plt.plot(ncellslist**3, 1 / np.sqrt(ncellslist**3), color="black", linestyle="dashdot", label="$1 / \sqrt{N}$")
# plt.plot(ncellslist**3, 1 / np.sqrt(ncellslist**3), color="black", linestyle="dashdot")
plt.legend(prop={'size': 0.65 * settings.fs}, loc="lower left")
plt.xlim(10**3, 100**3)
# plt.ylim(1e-3, 1e-1)
# plt.xticks(np.linspace(10, 100, 10, endpoint=True), [f"{i}$^3$" for i in np.linspace(10, 100, 10, endpoint=True).astype(int)])
plt.vlines(20**3, 1e-3, 1e-1, linestyle=":", color="black")
plt.yscale("log")
plt.xscale("log")
# plt.xticks([10**3, 100**3], [f"$10^3$", f"$100^3$"])
# plt.xticks([10**3, 100**3], [f"$10^3$", f"$100^3$"])
...
plt.plot(ncellslist**3, np.mean(k_none_ncells, axis=-1), color="black", label="no kernel", alpha=0)
plt.plot(ncellslist**3, np.mean(k_gauss_ncells, axis=-1), color="black", linestyle="--", label="gaussian kernel", alpha=0)
plt.plot(ncellslist**3, np.mean(k_grid_ncells, axis=-1), color="black", linestyle=":", linewidth=3, label="grid kernel", alpha=0)
plt.plot(ncellslist**3, 1 / np.sqrt(ncellslist**3), color="black", linestyle="dashdot", label="$1 / \sqrt{N}$", alpha=0)
...
# for sp in ax_ncells2.spines.itervalues():
    sp.set_visible(False)
ax_ncells2.spines["bottom"].set_visible(True)


# ax_ncells2 = ax_ncells.twiny()
# # ax_ncells2.set_aspect('equal', adjustable=None)
# ax_ncells2.xaxis.set_ticks_position("bottom")
# ax_ncells2.xaxis.set_label_position("bottom")


# # Offset the twin axis below the host
# ax_ncells2.spines["bottom"].set_position(("axes", -0.3))
...
# # ncellslist2 = np.linspace(10, 100, 4, endpoint=True).astype(int)
# ncellslist2 = np.array([10, 20, 50, 100])
# ax_ncells2.set_xscale("log")
# ax_ncells2.set_xlim(9.5, 105)
# ax_ncells2.set_xticks(ncellslist2)
# ax_ncells2.set_xticklabels(tick_function(ncellslist2))
# ax_ncells2.set_xlabel(r"average distance to nearest neighbour ($\mu m$)")
# plt.xlim(300, 30)
...
plt.text(ckpos[0], 0.090, f"{mannwhitneysig(pvals[0])}", fontsize=settings.fs, zorder=100, ha="center")
plt.plot(np.array([kpos[0], kpos[1]]), np.array([0.085, 0.085]), color="black", linewidth=1.5)
plt.plot(np.array([kpos[0], kpos[0] + 1e-6]), np.array([0.075, 0.085]), color="black", linewidth=1.5)
plt.plot(np.array([kpos[1], kpos[1] + 1e-6]), np.array([0.075, 0.085]), color="black", linewidth=1.5)
plt.plot(np.array([ckpos[0], ckpos[0] + 1e-6]), np.array([0.085, 0.090]), color="black", linewidth=1.5)

plt.text(ckpos[1], 0.16, f"{mannwhitneysig(pvals[1])}", fontsize=settings.fs, zorder=100, ha="center")
plt.plot(np.array([kpos[0], kpos[2]]), np.array([0.155, 0.155]), color="black", linewidth=1.5)
plt.plot(np.array([kpos[0], kpos[0] + 1e-6]), np.array([0.145, 0.155]), color="black", linewidth=1.5)
plt.plot(np.array([kpos[2], kpos[2] + 1e-6]), np.array([0.145, 0.155]), color="black", linewidth=1.5)
plt.plot(np.array([ckpos[1], ckpos[1] + 1e-6]), np.array([0.155, 0.16]), color="black", linewidth=1.5)

plt.text(ckpos[2], 0.135, f"{mannwhitneysig(pvals[2])}", fontsize=settings.fs, zorder=100, ha="center")
plt.plot(np.array([kpos[1], kpos[2]]), np.array([0.13, 0.13]), color="black", linewidth=1.5)
plt.plot(np.array([kpos[1], kpos[1] + 1e-6]), np.array([0.12, 0.13]), color="black", linewidth=1.5)
plt.plot(np.array([kpos[2], kpos[2] + 1e-6]), np.array([0.12, 0.13]), color="black", linewidth=1.5)
plt.plot(np.array([ckpos[2], ckpos[2] + 1e-6]), np.array([0.13, 0.135]), color="black", linewidth=1.5)
...
ax_pos(ax_kerngaussxy, 0., 0.01, 1.4, 1.4)
ax_pos(ax_kerngaussyz, -0.02, 0.01, 1.4, 1.4)
ax_pos(ax_kerngridxy, 0., 0.02, 1.4, 1.4)
ax_pos(ax_kerngridyz, -0.02, 0.02, 1.4, 1.4)

ax_pos(ax_pairnone, 0.01, 0, 1, 1.4)
ax_pos(ax_pairgauss, 0.01, 0.01, 1, 1.4)
ax_pos(ax_pairgrid, 0.01, 0.02, 1, 1.4)
...
# plt.tight_layout()
plot_fname = os.path.join(settings.loc, "plots", "randfield.png")
...
# plt.xlim(50,150)
# plt.ylim(50,150)
# plt.yticks(np.linspace(50, 150, 4, endpoint=True), np.linspace(0, 100, 4, endpoint=True)/ngridcells*3)
# plt.xticks(np.linspace(50, 150, 4, endpoint=True), np.linspace(0, 100, 4, endpoint=True)/ngridcells*3)
# plt.savefig("randfield_gridkernelyz.png")