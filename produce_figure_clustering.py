...
clustrw_afname = os.path.realpath(
    os.path.join(
            ...
            "data",
            "analytical",
            "clustering_rw_N1024_nphi360_T9000_dt0_01_v0_1_sigmatheta0_5_kappa10.npy"
        )
)
...
plt.plot(np.linspace(0,360,settings.phbins), amfr_cluststar, color="darkgray", linestyle='-', linewidth=3, zorder=0)
...
plt.plot(np.linspace(0,360, settings.phbins, endpoint=False), fr_mean, 'k', zorder = 10)
plt.plot(np.linspace(0,360, settings.phbins, endpoint=False), amfr_clustpwl, color="darkgray", linestyle='-', linewidth=3, zorder=0)
plt.xlabel(r'Movement direction ($^\circ$)', fontsize=fs)
plt.ylabel('Total firing \nrate (spk/s)', fontsize=fs)
plt.xticks([0,60,120,180,240,300,360],fontsize=fs)
# plt.xticks([])
...
plt.plot(
    [bar_dist * np.cos(2*np.pi*bar_ang/360), bar_dist * np.cos(2*np.pi*bar_ang/360) + bar_len], 
...
if not os.path.isfile(star_phd_fname):
    for ix,xx in enumerate(x):
        for iy,yy in enumerate(y):
            ...
            ph60s_starphd[ix, iy] = ph[np.argmin(
                    abs(ff-1./60*360/phbins)
                )]/2/np.pi*60
...
ax = plt.gca()
im = ax.pcolormesh(Xc,Yc,ph60s / np.pi * 30,cmap='twilight_shifted',vmin=-30, vmax=30, shading='auto')
plt.plot([0,1],[0,0],'k--')
plt.plot([0,0.5],[0,np.sqrt(3)/2],'k--')
plt.plot([0.5,1.5],[np.sqrt(3)/2,np.sqrt(3)/2],'k--')
plt.plot([1,1.5],[0,np.sqrt(3)/2],'k--')
